    ...
    <div>
      ...
      <div className="card" style={{ marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
        ...
        <input
          ...
          style={{ background: 'transparent', border: 'none', color: '#fff', outline: 'none', width: '100%', fontSize: '0.925rem' }}
        />
      </div>
      ...
      {loading ? (
        ...
        <div className="card" style={{ textAlign: 'center', padding: '3rem' }}>
          ...
          <h3 style={{ color: '#fff', marginBottom: '0.5rem' }}>No Approved Common Materials Yet</h3>
            ...
            <div key={cm.common_material_id} className="card" style={{ borderLeft: '4px solid var(--success)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
                <div>
                  ...
                  <h3 style={{ color: '#fff', fontSize: '1.2rem', marginTop: '0.25rem' }}>
              ...
              <div style={{ background: 'rgba(255, 255, 255, 0.02)', padding: '1rem', borderRadius: '8px', border: '1px solid var(--border-color)' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '0.75rem' }}>
                  {cm.source_materials && cm.source_materials.map((src, idx) => (
                    <div key={idx} style={{ background: 'rgba(255, 255, 255, 0.03)', padding: '0.75rem', borderRadius: '6px', border: '1px solid var(--border-color)' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.25rem' }}>
                        <span style={{ fontWeight: 700, color: 'var(--primary)', fontSize: '0.85rem' }}>{src.organization_id}</span>
                        <span style={{ fontWeight: 600, color: '#fff', fontSize: '0.85rem' }}>ID: {src.material_id}</span>
                      </div>
                      {src.description && (
